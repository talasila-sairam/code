...
$(document).ready(function() {
    $(".tree li a").click(function(){
    	 ...
    	 $(".tree li a").closest("li.parent_li").parent().parent().find(" > span").removeClass("onClickActive");
    	 $(this).closest("li.parent_li").parent().parent().find(" > span").addClass("onClickActive");
    	 $(".tree li a").closest("li.parent_li").parent().parent().parent().parent().find(" > span").removeClass("onClickActive");
    	 $(this).closest("li.parent_li").parent().parent().parent().parent().find(" > span").addClass("onClickActive");
    });
});
...
<input type="hidden" id="loginedUser" value="<%session.getAttribute("username");%>"/>
  ...
  <div id = "expandDiv" hidden = true>
  	<button id="expand" class="form-control" onclick = "expandTheTree();" style = "height:53px;width:2%;" ><i class="icon icon-caret-right" aria-hidden="true"></i></button>
  </div>
    ...
    <div>
        ...
        <ul>
         <li> <span><a href="javascript:getRulesFieldwise('','','','','','','global','','','','','')"><i class="icon-cog"></i>RULES</a></span></li>
        	<li> <span><i class="icon-folder-open"></i> ${countryName}</span>
        	<ul>
			<li> <span><a href="javascript:getRulesFieldwise('${countryCode}','','','${countryName}','','','country','','','','','')"><i class="icon-cog"></i>RULES</a></span></li>
              <li> <span><i class="icon-plus-sign"></i> ${stateName}</span>
                <ul>
				<li><span><a href="javascript:getRulesFieldwise('${countryCode}','${stateFipsCode}','','${countryName}','${stateName}','','state','${states.stateCode}','','','','')"><i class="icon-cog"></i>RULES</a></span></li>
					  <li> <span><i class="icon-plus-sign"></i> ${countyName}</span>
					  <ul>
					  <li> <span><a href="javascript:getRulesFieldwise('${countries.countryCode}','${states.fipsCode}','${counties.countyFipsCode}','${countries.countryName}','${states.stateName}','${counties.countyName}','county','${states.stateCode}','','','','')"><i class="icon-cog"></i>RULES</a></span></li>
						<c:forEach items="${fieldsList}" var="fields">
								<li><span><a href="javascript:getRulesFieldwise('${countryCode}','${stateFipsCode}','${countyFipsCode}','${countryName}','${stateName}','${countyName}','field','${states.stateCode}','${fields.fieldName}','${fields.fieldNumber}','${fields.defaultFlag}','${fields.mappingFieldName}')"><i class="icon-leaf"></i> ${fields.fieldNumber}-${fields.fieldName}-(${fields.ruleCount})</a></span></li>
						</c:forEach>
					  </ul>
		              </li>
                </ul>
  ...
  <div id = "collapseDiv">
  	<button id="collapse" class="form-control" onclick = "collapseTheTree();" style = "height:53px;width:2%"><i class="icon icon-caret-left" aria-hidden="true"></i></button>
  </div>
